...
// DEBUG:	echo '<p>GET: '.$_GET['qNum'].'  POST: '.$_POST['qNum'];
    ...
    for ($c=0; $c<=($ch_max+1); $c++) {
          ...
          if ($c==0) 			         { $hc = 'NONE'; }
          elseif ($c==($ch_max+1)) { $hc = 'ALL';  }			
					else                     { $hc = $c;     }		
          $chapter .= '<option value="'.$c.'" '.$sel.'>'.$hc.'</option>';
    }
    $chapter .= '</select>';
...
$Nqs = count($qs); 
	...
	if ($Nqs){ 
	  $qid = $qs[$qindex][0];
  		...
  		$sts = '<center><table class="ITS_QINFO">'.
  		   '<tr><th style="color:blue">Title</th><th style="color:red">Image</th><th style="color:green">Category</th><th style="color:brown">Tags</th></tr>'.
  		   '<tr><td style="color:blue">'.$title.'</td><td style="color:red">'.$image.'</td><td style="color:green">'.$category.'</td><td style="color:brown">'.$tagList.'</td></tr>'.
  		   '</table></center>';
...
/*
<td><?php echo $Q->Q_type?></td>
<td><?php echo $Q->Q_title?></td>
<script type="text/javascript" src="js/ITS_question.js"></script>
*/
?>
	...
	//NABANITA START  
		...
		function renderQuestionAnswer(str){
			...
			if(matches!=null){
				...
				var ques_str = str.replace(myRegExp,replacement_str);
				//alert(ques_str);
				return ques_str;
			} else {
				//alert(str);
...
<body>
<div id="tagDialog" title="TAGS" style="display:none">
	<B>ALL</B> of your ITS records will be permanently deleted and cannot be recovered.
...
<?php echo $form.' &nbsp;Available: <b>'.$Nqs.'</b>';?>
</form>
<!------------------------------------->
<div class="ITS_navigate">
<input id="previousQuestion" type="button" onclick="ITS_QCONTROL('PREV','ITS_question_container')"  name="prev_question" value="&nbsp;<<&nbsp;" qid="<?php echo $qid;?>">&nbsp;
<?php echo '<input type="text" class="ITS_navigate" onkeypress=ITS_QCONTROL(\'TEXT\',\'ITS_question_container\') name="qNum" value="'.$qid.'" id="ITS_QCONTROL_TEXT" Q_num="'.$qid.'" Q_type="'.$qtype.'">';?>&nbsp;
<input id="nextQuestion" type="button" onclick="ITS_QCONTROL('NEXT','ITS_question_container')" name="next_question" value="&nbsp;&gt;&gt;&nbsp;">
</div>
<div id="metaContainer" class="ITS_meta">
<?php echo $sts;?>
</div>
<!------------------------------------->
<!---*******************************--->
...
<div id="maincontent">
<div id="ITS_question_container">
<input type="button" class="ITS_button" id="newQuestion" name="new" value="New">
<input type="button" class="ITS_button" onclick="ITS_QCONTROL_EDITMODE(this)" name="editMode" value="Edit" status="true">
<input type="button" class="ITS_button" id="cloneQuestion"  name="clone"  value="Clone">
<input type="button" class="ITS_button" id="deleteQuestion" name="delete" value="Delete">
<input type="button" class="ITS_button" id="importQuestion" name="import" value="Import QTI">
<input type="button" class="ITS_button" id="testme" name="test" value="test">
*/
// RENDER QUESTION
if (!empty($qid)) {
