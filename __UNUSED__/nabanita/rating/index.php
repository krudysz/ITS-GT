...
/*-- RATING module ------------------------------------*/
require_once ("classes/ITS_rating.php");
/*-----------------------------------------------------*/
...
<div id="accor">
	<div class="accor">
		<h3 class="hel" id="headerQuestion" view="<?php echo $role=='student';?>"><a href="#" class="headering">Questions: <span name="Question" ch="<?php echo $ch;?>"><?php echo 'Chapter '.$ch;?></span></a></h3>
		 <div class="ITS_ghost">		
				 <?php 
				 echo $screen->main();
	...
	<div class="accor">
		<h3 id="headerReview"><a href="#" class="header">Review: <span name="Review" ch="<?php echo $ch;?>"><?php echo 'Chapter '.$ch;?></span></a></h3>
		<div class="ITS_ghost">
   ...
   //$("#chapter_index4").css({background: '#ffffff',border: '2px solid #666666'}); 
	 ...
	 //--------------rating-------------------//
		...
		$("#ratings").stars({
			...
			{
				//$.post("ajax/ajax_rating.php", {rate: value}, function(data)
				$.get('ITS_screen_AJAX2.php', { ajax_args: "rating", ajax_data: value}, function(data)
				{
					rate = data;
					//$("#ajax_response").html(rate);
				});
			}			
		});
	   //RATING END	
        ...
        var qtype  = $(this).attr("qtype");
         ...
         //----------//
			...
			if (chosen) {
				...
				//alert(rate);
				//alert('ajax_data:' + qid+'~'+qtype+'~'+chosen+'~'+c+'~'+rate);
												
				//RATING START
				rate = rate.charAt(rate.length - 1);
				$.get('ITS_screen_AJAX2.php', { ajax_args: "recordChapterAnswer", ajax_data: qid+'~'+qtype+'~'+chosen+'~'+c+'~'+rate}, function(data) {
