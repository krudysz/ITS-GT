	...
	function render_user_answer($answer, $score, $dist, $config, $score_idx) {
		...
		switch ($config) {
			...
			case 2 : // NO SCORE
				...
				'<tr><td class="' . $style . '" style="text-align:right;width:150px">Your Answer&nbsp;</td><td class="' . $style . '" style="font-size:large;font-weight:bold;text-align:center;">' . $str . '</td></tr>' .
				'</table></center>';
				break;
				//----------------------//
			default :
				$user = '<table class="ITS_SCORE">' .
				'<tr><td class="ITS_SCORE" style="text-align:right">Your Answer</td><td class="ITS_SCORE">' . $answer . '</td></tr>' .
				'<tr><td class="ITS_SCORE" style="width:20%;text-align:right">Score</td><td class="ITS_SCORE">' . $score[0] . '</td></tr>' .
				'</table>';
	...
	function get_total_score($score, $qanswer, $qtype) {
		...
		switch (strtolower($qtype)) {
			...
			case 'm' :
				//-------------------------------  		
				// var_dump($score); //die();
				if (is_array($score)) {  $total_score = array_sum($score); }  //$score[0]
			...
			case 'm' :
				//-------------------------------					
					...
					for ($i = 0; $i < count($config); $i++) {
						 if (!empty($config[$i])) { $idx[abs($config[$i])-1] = $config[$i]; }
					}
					$N = 0;
	        ...
	        //echo '<p>CONFIG: '.implode('_',$config).'<p>'; //die();			
					...
					for ($v = 0; $v < count($answered); $v++) {	
					  //echo $answered[0].'+'.$idx.'<p>';
						if    (empty($idx)) { $score_arr[$v] = NULL; }
					  elseif ($idx[$v]<0) { $score_arr[$v] = NULL; }
						else {				
    					if (empty($answered[$v])) { 
    						   $score_arr[$v] = 0;
    					} else {
...
//echo 'N '.$Nvals.'<p>';				
        ...
        //echo  'SOLUTION: '.$solution;
				if (empty ($formula)) { $solution = 'NO SOLUTION EXISTS'; } 
				//eval("\$str = \"$str\";");
				...
				elseif (empty($solution)) { $solution = NULL; }
...
//echo '--debug---';				
  ...
  //----------------------------------------------------------------------------	
	...
	var_dump($data_arr);
	echo '</pre><hr>';
	echo 'Nanswers: '.$Nanswers.'   Choices: '.$choices;	//die('---');
    */
	...
	*/						
    ...
    //----------------------------------------------------------------------------	
		...
		switch (strtolower($qtype)) {
					...
					for ($a = 0; $a < $sc; $a++) {  // count($answer)
						...
						//echo '<p>null?-'.$score[$index][$a].'<p>';
  						...
  						if (empty ($answer[$a])) {
  							...
  							if   ( empty ($scr)) { $class = "ITS_null"; }
  						  else {
    						  //echo '<p>SCORE: '.$score[$score_idx][$a].'<p>';
    						  if ($scr[$a] == 0) { $class = 'ITS_incorrect'; $msg = 'Incorrect'; }
									else           { $class = 'ITS_correct';   $msg = '<b>Correct</b>';   }
  						  }
						  }
  						$list[$k]   = '<b>'.($ai+1).'.</b>';
			...
			default :
				//-------------------------------	
				if (is_null($score[0])){$ans = '<div class="ITS_feedback" style="background: #FF9">' . $answer . '</div>';}
				elseif ($score[0]==100){$ans = '<div class="ITS_feedback"><span class="ITS_correct">' . $answer . '</span>&nbsp;&nbsp;<b>Correct</b>&nbsp;</div>';} 
				elseif ($score[0]==0)  {$ans = '<div class="ITS_feedback"><span class="ITS_incorrect">' . $answer . '</span>&nbsp;&nbsp;Incorrect&nbsp;</div>';}
				else                   {$ans = '<div class="ITS_feedback"><span class="ITS_partial">' . $answer . '</span>&nbsp;&nbsp;Partial Credit&nbsp;</div>';}
				//-------------------------------		
		}
		return $ans;
	}
  ...
  /*echo '<pre>';print_r($stats);echo '</pre>';//die('---');
		*/					
					...
					$queryA = 'SELECT answers FROM webct WHERE id=' . $qid;
      				...
      				if (PEAR :: isError($resA)) {throw new Question_Control_Exception($res->getMessage());}
...
*/
		...
		for ($t = 0; $t < $Nterms; $t++) {
  		...
  		if ($stats_SHOW) {
            ...
            $query = 'SELECT question_id,answered,qtype,answers,comment,epochtime,duration,rating FROM stats_'.$this->id.',webct WHERE webct.id=stats_'.$this->id.'.question_id AND current_chapter="'.$chapter.'" ORDER BY stats_'.$this->id.'.'.$orderby;
					  ...
					  $query = 'SELECT question_id,answered,qtype,answers,comment,epochtime,duration,rating FROM stats_'.$this->id.',webct WHERE webct.id=stats_'.$this->id.'.question_id AND current_chapter="'.$chapter.'" AND category IN ("SPEN'.$chapter.'","PreLab0'.$chapter.'","Lab'.$chapter.'","Chapter'.$chapter.'"'.$other.') AND qtype IN ("MC","M","C") ORDER BY stats_'.$this->id.'.'.$orderby;
  ...
  $users = $res->fetchAll();
	...
	for ($h=0; $h<count($chs); $h++) { $header .= '<th>Mod. '.$chs[$h].'</th>'; }
	$header .= '</tr>';

	//die($header);
  $Estr = '<center><div class="file"><a href="'.$file_path.'" target="_blank"><img alt="Export To Excel" src="admin/excel_graphic.png" /></a><br><font color="blue">'.date("F j, Y, g:i a T",$tMay1).'</font></div> <table class="CPROFILE">'.$header;
	//			 			 
	foreach ($users as $key => $user){ //$users as $user){
	  //Calculating Scores for this User
		...
		for($j=0;$j<count($chs);$j++) {
			...
			if ($chs[$j] == 1)     { $other = ',"Complex"';          } 
			elseif ($chs[$j] == 13){ $other = ',"PEZ","chapter7DM"'; }
		    else 			       { $other = '';                    }
					
			//$q1 = 'SELECT SUM(score) AS sum FROM '.$usertable.' WHERE current_chapter='.$chs[$j].' AND epochtime > '.$epochtime;
			$q1 = 'SELECT SUM(score) AS sum FROM '.$usertable.',webct WHERE '.$usertable.'.question_id=webct.id AND current_chapter="'.$chs[$j].'" AND category IN ("SPEN'.$chs[$j].'","PreLab0'.$chs[$j].'","Lab'.$chs[$j].'","Chapter'.$chs[$j].'"'.$other.') AND '.$usertable.'.score IS NOT NULL AND epochtime > '.$epochtime;
			//echo '<p>'.$q1.'</p>';die();
			$r1    = mysql_query($q1);		
    ...
    for ($t=0; $t<count($chs); $t++) {
      $td .= '<td style="text-align:right">'.$totalscore[$key][$t].'</td>';
    }
    $Estr.= '<tr>'.
			'<td style="font-size:8pt">'.$user[0].'</td>'.
