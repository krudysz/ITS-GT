...
class ITS_screen2 {
    ...
    function __construct($id,$term,$role) {
        ...
        //
        $this->screen = 2;
        $this->mode = 'review'; //'question' | 'review'
	...
	*----------------------------------------------------------*/
    ...
    function getAnswer($qid,$qtype,$answered,$config) {     //getUserAnswer
        ...
        $stats = $tr->get_question_stats($DATA, $qtype, 1);
    ...
    function getQuestionQuery($chapter) {
        //=====================================================================//
        $resource_name = $this->chapter_number;
        ...
        $query = 'SELECT question_id,answered,qtype,answers,rating,comment,epochtime FROM stats_'.$this->id.',webct WHERE webct.id=stats_'.$this->id.'.question_id AND webct.category IN ("SPEN'.$resource_name.'","PreLab0'.$resource_name.'","Chapter'.$resource_name.'","Lab'.$resource_name.'"'.$other.') OR category RLIKE "-Mod'.$resource_name.'" AND current_chapter IN ('.$this->chapter_number.',-'.$this->chapter_number.') AND epochtime > '.$this->epochtime;
    ...
    function reviewNav($qid,$qAvail) {
        ...
        $nav      = $this->getNavigation($nav_info,$qid);
            ...
            //-------------------------------//
		      ...
		      $la = sprintf("%02d",$resource_name);
                ...
                */
                ...
                $resource_source = 'category IN ("SPEN'.$resource_name.'","PreLab0'.$resource_name.'","Chapter'.$resource_name.'","Lab'.$resource_name.'"'.$other.') OR category RLIKE "-Mod'.$chs[$j].'" AND qtype IN ("M","MC","C")';
            ...
            */
            if (empty($qarr)) {
                ...
                switch ($resource) {
                    ...
                    //-------------------------------//
                    default:
                        $query = 'SELECT id,qtype FROM webct WHERE id IN ('.$ques_list.') AND id NOT IN (SELECT question_id FROM stats_'.$this->id.' WHERE score IS NOT NULL AND current_chapter='.$resource_name.' AND epochtime > '.$this->epochtime.')'; //###!!!
              ...
              //echo $resources.'<p>';die('done');
                    */
                    ...
                    //echo $this->role.'<hr><div style="background:yellow">'.$answer.'</div>';
                    ...
                    if ($this->role == 'admin' OR $this->role == 'instructor') {
                        ...
                        $ques_ref = 'bbb<div class="ITS_ADMINx" style="border:3px solid red">'.$tb->str.'<br>'.$qEdit.'</div>';
                    }
                    else {
                        $ques_ref = '';
                    }
